...
3. backend/README.md
4. AGENTS.md (legacy reference, planned for deletion later)
5. README.md (human-facing, may be stale)
...
- orval.config.cjs currently points output and mutator paths to frontend/src/api/*, which does not match current frontend structure.
- package.json script generate:api:watch points to ./orval.config.js, but only orval.config.cjs exists.
- Before regenerating clients for production work, align Orval output paths with frontend/src/shared/api (target, schemas, mutator path).

If backend serializers or API shapes change, regenerate frontend API clients in the same change set.
