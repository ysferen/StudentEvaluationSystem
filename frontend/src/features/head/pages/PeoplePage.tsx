...
import { Check, Clipboard, KeyRound, Loader2, UserPlus, X } from 'lucide-react'
import { axiosInstance } from '@/shared/api/mutator'
import { Card } from '@/components/ui/custom/Card'

type User = { id: number; username: string; first_name: string; last_name: string; role: string; title?: string; must_change_password?: boolean }
...
const PeoplePage = () => {
  ...
  return <div className="space-y-6 pb-12">
    ...
    <Card className="p-5"><h2 className="mb-3 font-semibold">Instructors</h2><div className="space-y-2">{instructors.map(user => <div key={user.id} className="flex flex-wrap items-center gap-3 rounded-xl bg-secondary-50 px-4 py-3"><span className="font-medium">{user.title && `${user.title} `}{user.first_name} {user.last_name}</span><span className="text-sm text-secondary-500">@{user.username}</span>{user.must_change_password && <span className="rounded-full bg-amber-100 px-2 py-1 text-xs font-medium text-amber-700">Password change required</span>}<button disabled={busyUser === user.id} className="ml-auto inline-flex items-center gap-2 rounded-lg bg-white px-3 py-2 text-sm font-medium text-primary-700 shadow-sm hover:bg-primary-50 disabled:opacity-50" onClick={() => void reset(user)}>{busyUser === user.id && <Loader2 className="h-4 w-4 animate-spin" />}Reset password</button></div>)}</div></Card>
