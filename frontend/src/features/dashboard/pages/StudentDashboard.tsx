...
const StudentDashboard = () => {
  ...
  return <div className="space-y-8">
    ...
    <section className="grid gap-6 xl:grid-cols-2">
      <Suspense fallback={<div className="animate-pulse bg-gray-200 rounded-lg" style={{height: 360}} />}><ChartWidget title="Course performance" subtitle="Your weighted average in each current course" type="bar" series={[{ name: 'Score', data: courseChart.map(item => item.score) }]} options={{ colors: ['#0891b2'], xaxis: { categories: courseChart.map(item => item.code) }, yaxis: { min: 0, max: 100 }, plotOptions: { bar: { borderRadius: 6, columnWidth: '48%' } }, dataLabels: { enabled: false } }} height={360} /></Suspense>
      <Suspense fallback={<div className="animate-pulse bg-gray-200 rounded-lg" style={{height: 360}} />}><ChartWidget title="Program outcome progress" subtitle="Your achievement by program outcome" type="radar" series={[{ name: 'Achievement', data: poScores.map(score => Math.round(score.score ?? 0)) }]} options={{ colors: ['#0d9488'], xaxis: { categories: poScores.map(score => score.program_outcome.code) }, yaxis: { min: 0, max: 100, tickAmount: 5 }, stroke: { width: 2 }, fill: { opacity: 0.25 }, markers: { size: 4 } }} height={360} /></Suspense>
    </section>
