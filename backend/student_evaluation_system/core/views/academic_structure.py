...
from ..serializers import (
    ...
    FileValidationResponseSerializer,
    CourseAverageSerializer,
    LearningOutcomeAverageSerializer,
)
...
class StudentLearningOutcomeScoreViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def course_averages(self, request):
        ...
        if student_id and course_id:
            ...
            enrollments = CourseEnrollment.objects.filter(student_id=student_id).values_list("course_id", flat=True)
        ...
        for cid in course_ids:
            # Build base query
            lo_scores_query = StudentLearningOutcomeScore.objects.filter(learning_outcome__course_id=cid)
    ...
    def lo_averages(self, request):
        ...
        if not course_id:
            return Response({"error": "course query parameter is required"}, status=status.HTTP_400_BAD_REQUEST)

        # Get all LO scores for this course grouped by learning outcome
        lo_averages = (
            StudentLearningOutcomeScore.objects.filter(learning_outcome__course_id=course_id)
...
class StudentProgramOutcomeScoreViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    queryset = StudentProgramOutcomeScore.objects.select_related("student", "program_outcome", "term").all()
    ...
    def get_queryset(self):
        ...
        # Instructors see scores of students in their courses
            ...
            instructor_course_ids = user.taught_courses.values_list("id", flat=True)
            student_ids = CourseEnrollment.objects.filter(course_id__in=instructor_course_ids).values_list(
    ...
    def _get_course_by_code_and_term(self, course_code: str, term_id: int):
        ...
        try:
            ...
            if available_courses.exists():
                ...
                return Response({"error": f'Course with code "{course_code}" not found.'}, status=status.HTTP_400_BAD_REQUEST)
...
class AssignmentScoresImportViewSet(BaseFileImportViewSet):
    ...
    def upload(self, request):
            ...
            return Response(
                {
                    ...
                    "description": (
                        "POST a file here to import assignment scores from Turkish Excel format. "
                    ...
                    "required_fields": {"file": "File to upload (.xlsx, .xls) in Turkish Excel format"},
                    "expected_columns": {
                        "student_id": ["öğrenci no", "No_0833AB", "Öğrenci No_0833AB"],
                        "first_name": ["adı", "Adı_0833AB"],
                        "last_name": ["soyadı", "Soyadı_0833AB"],
                        "assessment_columns": (
                            "Pattern: AssessmentName(%weight)_0833AB (e.g., Midterm 1(%25)_0833AB, Project(%40)_0833AB)"
                        ),
                    },
                    "example_usage": [
                    ...
                    "example_curl": [
                        "curl -X POST \\",
                        (
                            '  "http://localhost:8000/api/core/file-import/assignment-scores/'
                            'upload/?course_code=MATH101&term_id=3" \\'
                        ),
        ...
        try:
            ...
            try:
                ...
                return Response({"error": "term_id must be a valid integer"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            results = self.file_service.import_assignment_scores(course_code=course_code, term_id=term_id)

            return Response(
                {
                    "message": f"Assignment scores imported successfully for course {course.code} ({course.term.name})",
                    "course_info": {"code": course.code, "name": course.name, "term": course.term.name},
                    "results": results,
    ...
    def _validate_get_request(self):
        """Return validation help documentation for GET requests."""
        return Response(
            {
                "message": ("Assignment Scores Validation Endpoint (Turkish Format)"),
                "description": (
                    "POST a file here to validate its format for assignment scores import from Turkish Excel format."
                ),
                "required_query_parameters": {
                    "course_code": "Code of the course (REQUIRED for validation)",
                    "term_id": ("ID of the academic term (REQUIRED for validation)"),
                },
                "required_fields": {"file": ("File to validate (.xlsx, .xls) in Turkish Excel format")},
                "validates": [
                ...
                "example_curl": [
                    "curl -X POST \\",
                    (
                        '"http://localhost:8000/api/core/file-import/'
                        "assignment-scores/validate/"
                        '?course_code=MATH101&term_id=3" \\'
                    ),
                    '-H "Content-Type: multipart/form-data" \\',
                    ('-F "file=@assignment_scores_turkish.xlsx"'),
                ],
    ...
    def _validate_post_request(self, request):
        """Validate POST request for assignment scores validation."""
        if "file" not in request.FILES:
        ...
        try:
            ...
            return None, Response({"error": "term_id must be a valid integer"}, status=status.HTTP_400_BAD_REQUEST)

        course = self._get_course_by_code_and_term(course_code, term_id)
        if isinstance(course, Response):
            return None, course

        return (file_obj, course), None

    def _build_validation_response(self, validation_result, course):
        """Build validation response from validation result."""
        response_data = {
    ...
    def validate(self, request):
        ...
        """
        if request.method == "GET":
            return self._validate_get_request()

        try:
            data, error_response = self._validate_post_request(request)
            if error_response:
                return error_response

            file_obj, course = data
            validation_result = AssignmentScoreValidator.validate_complete(file_obj, course)
            return self._build_validation_response(validation_result, course)
...
class LearningOutcomesImportViewSet(BaseFileImportViewSet):
    ...
    def upload(self, request):
        ...
        try:
            ...
            return Response(
                {"message": "Learning outcomes import completed successfully", "results": results}, status=status.HTTP_200_OK
            )
            ...
            return Response(
                {
                    ...
                    "required_fields": {"file": "File to validate (.xlsx, .xls, .csv)"},
                    "expected_columns": ["code", "description", "course_code"],
                    "example_curl": [
                        "curl -X POST \\",
                        '  -H "Content-Type: multipart/form-data" \\',
            ...
            return Response(
                {
                    ...
                    "file_info": {"name": file_obj.name, "size": file_obj.size, "format": file_service.detect_file_format()},
                    "expected_columns": ["code", "description", "course_code"],
                },
                status=status.HTTP_200_OK,
            )
...
@extend_schema_view(
    upload=extend_schema(
        ...
        request={
            "multipart/form-data": {
                "type": "object",
                "properties": {"file": {"type": "string", "format": "binary"}, "sheet_name": {"type": "string"}},
                "required": ["file"],
            }
        },
        responses={200: FileImportResponseSerializer, 400: dict},
...
class ProgramOutcomesImportViewSet(BaseFileImportViewSet):
    ...
    def upload(self, request):
        ...
        try:
            ...
            return Response(
                {"message": "Program outcomes import completed successfully", "results": results}, status=status.HTTP_200_OK
            )
