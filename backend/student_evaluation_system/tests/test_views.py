...
class TestProgramOutcomeViewSet:
    ...
    def test_list_program_outcomes(self, api_client, sample_course):
        ...
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data["results"]) >= len(pos)
...
class TestLearningOutcomeViewSet:
    ...
    def test_list_learning_outcomes(self, api_client, sample_course):
        ...
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data["results"]) >= len(los)
...
class TestLearningOutcomeProgramOutcomeMappingViewSet:
    ...
    def test_list_mappings(self, api_client, sample_course):
        ...
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data["results"]) >= len(mappings)
    ...
    def test_create_mapping(self, authenticated_client, sample_course):
        ...
        lo = LearningOutcome.objects.create(code="LO3", description="New Learning Outcome for testing", course=course)
        po = ProgramOutcome.objects.create(
            code="PO3", description="New Program Outcome for testing", program=program, term=term
        )

        url = reverse("lo-po-mapping-list")
        data = {"course": course.pk, "learning_outcome_id": lo.pk, "program_outcome_id": po.pk, "weight": 0.8}
        response = client.post(url, data)

        assert response.status_code == status.HTTP_201_CREATED
        assert LearningOutcomeProgramOutcomeMapping.objects.filter(
            course=course, learning_outcome=lo, program_outcome=po
        ).exists()
...
class TestStudentLearningOutcomeScoreViewSet:
    ...
    def test_list_student_lo_scores(self, api_client, sample_course, student_factory):
        ...
        StudentLearningOutcomeScore.objects.create(student=student.user, learning_outcome=lo, score=85.0)
    ...
    def test_filter_scores_by_student(self, api_client, sample_course, student_factory):
        ...
        StudentLearningOutcomeScore.objects.create(student=student.user, learning_outcome=lo, score=90.0)
    ...
    def test_filter_scores_by_course(self, api_client, sample_course, student_factory):
        ...
        StudentLearningOutcomeScore.objects.create(student=student.user, learning_outcome=lo, score=88.0)
...
class TestStudentProgramOutcomeScoreViewSet:
    ...
    def test_list_student_po_scores(self, api_client, db_setup, student_factory):
        ...
        po = ProgramOutcome.objects.create(code="PO1", description="Test PO", program=program, term=term)

        from core.models import StudentProgramOutcomeScore

        StudentProgramOutcomeScore.objects.create(student=student.user, program_outcome=po, score=85.0, term=term)
